    ...
    <div className={`mb-0 ${className?.includes('mb-') ? '' : ''}`}> {/* Removed default mb-4 to let parent control spacing if needed, actually kept generic container styling */}
      ...
      <div className="relative">
        ...
        <input
          id={id}
          type={inputType}
          className={`block w-full px-4 py-3 text-white border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500 transition-colors duration-200
            ${baseBgClass}
            ${icon ? 'pl-10' : ''}
