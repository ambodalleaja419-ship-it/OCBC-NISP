...
const FAQ_DATA: FaqItem[] = [
  {
    ...
    answer: "Member wajib menyelesaikan biaya aktivasi ulang itu untuk mencairkan (USD ke RUPIAH)"
  },
  {
  ...
  },
  {
    ...
    answer: "OCBC NISP menawarkan tiga mata uang dasar untuk Anda perdagangkan dan investasikan. Anda dapat memiliki beberapa akun untuk setiap mata uang dasar."
  },
  {
    ...
    <div className="container mx-auto px-4 lg:px-8 max-w-5xl">
        ...
        {FAQ_DATA.map((item, index) => (
          <div 
            ...
            className={`border rounded-lg overflow-hidden transition-all duration-300 ${
              openIndex === index 
                ? 'bg-[#1E2329] border-primary/50 shadow-lg shadow-black/40' 
                : 'bg-darkblue2 border-borderGray hover:border-gray-600 hover:bg-[#1A1F29]'
            }`}
          >
            <button
            ...
            >
              <span className={`font-medium text-sm sm:text-lg transition-colors duration-200 ${
                  openIndex === index ? 'text-primary' : 'text-gray-200 group-hover:text-white'
              }`}>
                {item.question}
              </span>
              <span className={`flex-shrink-0 ml-4 p-1 rounded-full transition-all duration-300 ${
                  openIndex === index ? 'bg-primary/20 text-primary rotate-180' : 'text-gray-500 bg-gray-800'
              }`}>
            ...
            >
              <div className="px-5 pb-6 text-gray-300 text-sm sm:text-base leading-relaxed border-t border-gray-700/30 pt-4">
                {item.answer}
              </div>
            </div>
          </div>
      ...
      <div className="text-center mt-8 pb-8">
          <p className="text-gray-500 text-sm">
              Masih tidak menemukan jawaban? <span className="text-primary cursor-pointer hover:underline font-medium">Hubungi Layanan Pelanggan</span>
          </p>
      </div>
    </div>
  );
