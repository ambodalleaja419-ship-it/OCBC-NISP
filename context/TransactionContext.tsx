...
export const TransactionProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  ...
  const withdraw = async (
    ...
    if (success) {
      addNotification(`Penarikan Rp ${amount.toLocaleString('id-ID')} sukses, silahkan lanjut ke admin OCBC NISP untuk info lebih lanjut.`);
      refreshUser();
      fetchTransactions();
    } else {
  ...
  const adminUpdateUserBalance = async (userId: string, amount: number, type: 'set' | 'add') => {
    ...
    if (targetUser) {
        ...
        await authService.addUserNotification(userId, `Your balance has been updated by an administrator. New balance: Rp ${newBalance.toLocaleString('id-ID')}`);
        if (user?.id === userId) {
            refreshUser();
        }
    }
  ...
  const updateCompanyBankInfoList = async (infoList: CompanyBankInfo[]) => {
    ...
    addNotification('Company bank information updated by Admin.');
  };
